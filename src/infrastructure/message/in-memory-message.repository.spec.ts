...
describe('InMemoryMessageRepository', () => {
  ...
  it('should find by period (inclusive)', async () => {
    ...
    await repo.save(
      new Message('1', 'c1', 'a', new Date('2026-02-01T10:00:00Z'), MessageStatus.SENT),
    );
    await repo.save(
      new Message('2', 'c2', 'a', new Date('2026-02-10T10:00:00Z'), MessageStatus.SENT),
    );
    await repo.save(
      new Message('3', 'c3', 'a', new Date('2026-03-01T10:00:00Z'), MessageStatus.SENT),
    );
  ...
  it('should return empty array when findByPeriod has no matches', async () => {
    ...
    await repo.save(
      new Message('1', 'c1', 'a', new Date('2026-01-01T10:00:00Z'), MessageStatus.SENT),
    );
  ...
  it('should update an existing message', async () => {
    ...
    expect(stored!.status).toBe(MessageStatus.RECEIVED);
  });
